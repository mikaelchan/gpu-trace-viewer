...
<style>
:root { color-scheme: light; --bg:#f6f7f9; --panel:#fff; --line:#d9dee7; --text:#171b22; --muted:#657084; --accent:#0f766e; --soft:#e6f3f1; }
...
.table-wrap { overflow:auto; border:1px solid var(--line); border-radius:8px; background:var(--panel); max-height:calc(100vh - 235px); }
...
@media (max-width:1000px) { .toolbar { grid-template-columns:1fr 1fr; } .metrics { grid-template-columns:1fr 1fr; } .split { grid-template-columns:1fr; } }
</style>
</head>
<body>
...
function renderCalls(rows) { hideContext(); $("calls").tHead.innerHTML = `<tr><th>Order</th><th>Index</th><th>Dev</th><th>Stream</th><th>Device us</th><th>Occ %</th><th>Free us</th><th>Total us</th><th class="name">Kernel</th></tr>`; $("calls").tBodies[0].innerHTML = rows.map(r => `<tr data-call-order="${r.call_order}"><td>${r.call_order}</td><td>${r.op_call_index}</td><td>${esc(r.device)}</td><td>${esc(r.stream)}</td><td>${num(r.device_time_us)}</td><td>${maybe(r.occupancy_pct)}</td><td>${num(r.free_time_us)}</td><td>${num(r.total_time_us)}</td><td class="name" title="${esc(r.op_name)}">${esc(r.op_name)}</td></tr>`).join(""); [...$("calls").querySelectorAll("tr[data-call-order]")].forEach(tr => { tr.addEventListener("mouseenter", e => showContext(tr, e)); tr.addEventListener("mousemove", positionContext); tr.addEventListener("mouseleave", hideContext); }); }
