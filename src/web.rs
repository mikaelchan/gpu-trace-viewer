...
function renderCalls(rows) { hideContext(); $("calls").tHead.innerHTML = `<tr><th>Order</th><th>Dev</th><th>Stream</th><th>Device us</th><th>Occ %</th><th>Free us</th><th>Total us</th><th>Blocks/SM</th><th>Warps/SM</th><th>Shared mem</th><th>Grid</th><th>Block</th><th class="name">Kernel</th></tr>`; $("calls").tBodies[0].innerHTML = rows.map(r => `<tr data-call-order="${r.call_order}"><td>${r.call_order}</td><td>${esc(r.device)}</td><td>${esc(r.stream)}</td><td>${num(r.device_time_us)}</td><td>${maybe(r.occupancy_pct)}</td><td>${num(r.free_time_us)}</td><td>${num(r.total_time_us)}</td><td>${maybe(r.blocks_per_sm)}</td><td>${maybe(r.warps_per_sm)}</td><td>${maybe(r.shared_memory)}</td><td>${esc(r.grid)}</td><td>${esc(r.block)}</td><td class="name" title="${esc(r.op_name)}">${esc(r.op_name)}</td></tr>`).join(""); [...$("calls").querySelectorAll("tr[data-call-order]")].forEach(tr => { tr.addEventListener("mouseenter", e => showContext(tr, e)); tr.addEventListener("mousemove", positionContext); tr.addEventListener("mouseleave", hideContext); }); }
